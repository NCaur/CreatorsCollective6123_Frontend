...
const ProjectTasksScreen = () => {
  ...
  const fetchTasks = async () => {
        ...
        if (rows.length > 0) {
          const taskslist = rows.map((task) => {
            ...
            return {
              ...
              assignedTo: task.assignedToUserId?._id,
              email: task.assignedToUserId?.email,
              status: task.status,
    ...
    axios.get(`${BASE_URL}/api/${projid}`)
      .then((response) => {
        const projects = response.data;

        setProjectName('')
        //const proHeader = 'ID' + projects._id + '#' + projects.title
        const proHeader = projects.title
        //console.log("proHeader", proHeader)
  ...
  const fetchProjects = () => {
    // Replace this with the actual project ID you want to fetch tasks for
    axios.get(`${BASE_URL}/api`, { params: { "isAdmin": isAdmin, "userId": userId } })
      .then((response) => {
        const projects = response.data;
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (rows.length > 0) {
        const sortedUsers = rows.sort((a, b) => b.id - a.id)
  ...
  const onDelete = async (taskId) => {
    ...
    // Update the tasks state to reflect the changes
      ...
      try {
        await axios.delete(`${BASE_URL}/api/${taskId}`);
        setProjectList(projectList.filter((task) => task.id !== taskId));
      } catch (error) {
